...
> 自研 GPU 芯片要不要写汇编器？用 LLVM MC 能免掉解析器/编码器/反汇编器的重复实现。
...
## 2. LLVM IR 再深一层：三个关键语法

LLVM IR 的基础你已经知道：SSA + 强类型 + 虚拟寄存器。这里补三个"看懂真实 .ll 文件必备"的语法。
...
> 因为只有这样，优化器（死代码消除、重命名这些 pass）才能确定
...
你在 toycc 里写过融合 pass。LLVM 的 pass 体系是**同一思想、工业级实现**。
...
内存规划用的"生命周期 + 复用"思想，LLVM 寄存器分配是它的**升级版**：
...
> 1. **可逆**：字节 → MCInst → 文本（反汇编器免费）
> 2. **可分析**：调试器/模拟器都吃 MCInst（调试器就能
>    直接复用 MC 的反汇编输出，不用自己再写一套）
> 3. **可重定位**：MCInst 里的符号（如 `call foo`）先留占位，
>    链接时填地址。
>
> 你的芯片加 LLVM 后端时，编码规则写在 TableGen：`let AsmString` 旁边
> 用位段写编码（如 `let Inst{31-26} = 0b000000; let Inst{15-11} = rd;`），
> MC 自动生成编码器和解码器，**编码/解码不一致的 bug 在生成阶段就被消掉了**。
...
5. 实现 C++ 钩子: TargetLowering(合法化/类型系统) / TargetInstrInfo(指令选择)
6. 用 MC 自动生成汇编器/反汇编器
7. 跑 LLVM 的 lit 测试 (FileCheck 验证生成的汇编对不对)
```

**实际工作量**：一个"能跑简单算术"的最小后端，熟练的人 1~2 周；
一个"生产可用"的后端，数月。**但解析器/编码器/反汇编器都免掉了，
不用从零写**。
...
- 如果芯片像 GPU：可能要自己设计一层"类 PTX" + 自己的 ptxas
...
完成以下四项才算通过本课（每题都能用 `python -m course.runner 25` 验证）：

1. 手算 `%v = phi i32 [ %x, %then ], [ %y, %else ]` 在两条路径上的取值，并说出 phi 的两个硬性约束（incoming 与前驱一一对应、只能出现在块头）；
2. 手算 `getelementptr i32, ptr base, i64 3` 的地址偏移，并解释"GEP 不读内存"对别名分析的意义；
3. 解释 PreservedAnalyses 三档（all / none / preserve）的语义，并举一个"谎报 all()"导致错误优化的场景；
4. 默写后端流水线四段（指令选择 → 调度 → 寄存器分配 → MC 编码），各写一句它消费什么、产出什么。

---

## 8. 扩展阅读 A：为什么 LLVM 后端这么“重”？

你可能会问：toycc 的 codegen 是几十行量级的简化实现，LLVM 后端为什么这么复杂？
...
GlobalISel 在 AArch64 的 -O0 已是默认路径；RISC-V 后端则长期停留在实验态，直到近年才转正。你做自研芯片时，
