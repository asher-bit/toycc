...
>
> 用 A100（40GB 版，带宽 1.55 TB/s），算 1 亿元素 `vector_add`：
> - 数据量：`3 × 1e8 × 4 字节 = 1.2 GB`
> - 理论最短时间：`1.2 GB / 1.55 TB/s ≈ 0.77 ms`
>
> 现在用 roofline 核对：计算强度 0.08 FLOP/Byte，算力墙在天上
> （要超过 `19.5 TFLOP/s` 得强度 ≥ `19.5e12/1.55e12 ≈ 12.6 FLOP/Byte`），
> 0.08 离 12.6 差了 150 多倍——**绝对带宽受限，铁定撞带宽墙**。
>
> 那么结论就非常实用了：
> - 你**不该花一周去"优化" vector_add**——它的天花板就在 0.77 ms，
>   再怎么改也跳不出带宽墙
> - 真正该做的是**提高有效带宽利用率**：向量化 `float4`（一次搬 16 字节，
>   减少指令数）、大 block 连续跑，让实测接近 0.77 ms
...
>    相邻线程读不同 k（就是前面说的"一半坏"）→ 每个 warp 32 个
>    线程分散到 32 个 k 列 → 一次请求要被拆成几十次事务。
> 3. 决定改法：**先 tile 到共享内存**，再测——
