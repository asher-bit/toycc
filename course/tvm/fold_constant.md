# TVM Relax 源码精读专题：`fold_constant.cc`
...
| 现象 | 根因 | 定位手段 |
|---|---|---|
| 该折叠的没折叠 | 常量条件没满足（输入非 Constant、shape 不完整） | 逐条核对"可折叠"的判定条件 |
| 折叠后结果错 | 大输出算子的上限阈值判断错（如 1024 边界） | 单测卡在阈值两侧各一例 |
| 每次折叠都重新编译 | 构建缓存没生效，key 不一致 | 检查 `GetCachedBuild` 的 key 构成 |
| legalize 后失去折叠机会 | 高层 Op 没先 legalize 成 call_tir | 跑 legalize → fold 的顺序实验 |

## 本章检查点

完成以下四项才算通过本章：

1. 写出 `FoldConstant` 判定"可以折叠"的完整条件链，并各举一个不满足条件的反例；
2. 手算：一个 `zeros([2048])` 与一个 `zeros([512])` 在默认阈值 1024 下各走哪条路径（折叠 / 跳过），并说明阈值存在的理由；
3. 用 toycc 的 `ConstantFoldPass` 与本文对照，列出三处相同的设计决策（如"权重是常量、输入不是"的判定）；
4. 给"实验五"写一个能自动断言的测试（同一 PrimFunc 出现两次，构建日志只出现一次）。
