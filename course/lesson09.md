# 第 9 课：真实 TVM（下）——把 toycc 的每一步在真框架里跑一遍

> 本课风格：扫完剩余 4 个源码文件 → 安装 TVM → 跑一条真实管线。
> 完成本课，你就正式"毕业"：能看、能跑、能上手真实编译器了。

完整的 Relax Pass 阅读顺序和后续源码清单见：[真实 TVM 源码精读专题](pass_roadmap.md)。
...
**下一步**：第 10 课（最后一课）——从"学"到"做"：怎么真正参与编译器开发流程，
从给 toycc 加功能，到给 TVM 提第一个 PR。

---

## 7. 深层拓展 A：pass 基础设施的"三张通行证"
...
## 8. 深层拓展 B：Sequential 的"隐藏能力"——依赖、配置、调试
...
## 9. 深层拓展 C：Relax 的"三阶段"编译流程（完整图景）
