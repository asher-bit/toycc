...
>
> 手算本课自己的一个交接：`conv1_lt_0` 被 `conv1` 消费，它必须活到 t=1——
> 也就是 `conv1` 执行完。所以 t=1 时不能复用它的缓冲（下面 4.3 的模拟里，
> `buf0` 在 t=1 还没死，只能新开 `buf1`）。**少算一个消费者 → 提前释放 →
> 后面读到脏数据**，这是内存优化最经典的 bug。
>
> 所以编译器必须**精确统计每个张量最后一次被消费的时刻**——这里不需要维护
> 完整的消费者列表，直接枚举每个算子的输入（`node.inputs`）取最大时刻即可。
> 这也是"活跃变量分析"的雏形——寄存器分配、内存规划用的都是同一套
> "最后使用"逻辑。
>
...
完成以下四项才算通过本课（每题都能用 `python -m course.runner 6` 验证）：

1. 手算：对 `conv1 → relu1 → conv2` 链，写出每个张量的"生死区间"（生 = 产生它的算子序号，死 = 最后一个消费者的序号），指出哪个缓冲区可以被复用；
2. 用 runner 6 的输出核对：峰值分配 848 元素 → 复用后 448 元素的 47% 是怎么算出来的（写出 (848−448)/848）；
3. 说出 first-fit 与 best-fit 的区别，以及各自的一个优缺点；
4. 解释为什么对齐约束（如 4 字节对齐）会让"复用"失败，即使大小看起来够用。
