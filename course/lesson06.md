# 第 6 课：内存规划——6 个张量塞进 2 个缓冲区，省 70%
...
> 这是内存优化最经典的 bug。
>
> 所以编译器必须**精确统计每个张量的消费者**（第 1 课的 `consumers()` 就在这里用）：
> 死亡 = 拓扑序最大的那个消费者的时刻。这也是第 12 课"活跃变量分析"的雏形——
> 寄存器分配、内存规划用的都是同一套"最后使用"逻辑。
>
> 真实 TVM 的 `KillAfterLastUse`（第 6 课对照）就是精确做这件事：
...
**看出来了没？** 这些生命周期**一个挨一个，几乎没有重叠**！
因为图是一条直线，每个算子算完，它的输出要么被下一个算子吃掉、
要么就不再需要了。

- 朴素方案：把 6 个都存下来 = 848 元素
- 复用方案：同一时刻只有 1~2 个活着 → 峰值只有 256（那个最大的 conv1）

**内存省 70% 就是这么来的：让"死掉的"缓冲区被下一个张量接管。**
...
> TVM 的 `KillAfterLastUse` pass 干的就是这件事（第 8 课会看到源码）。
...
t1: conv1 需要 256
    buf0 还活着(died_at=1 >= 1? 不, died_at=1, t=1, 要求 died_at < 1 才可复用 → 不行)
    → 新开 buf1, size=256
...
**峰值 256 恰好是那个最大的 conv1**——因为任何时候最多同时活跃 256 元素。
"峰值 = 最大活跃张量之和"就是内存规划的目标函数。
...
- 生死由"出生时刻 + 最后一个消费者"决定
- 复用条件两句话：**死得够早、装得下**
- 朴素 848 → 复用 256，省 70%——峰值 = 最大活跃张量之和
...
## 10. 深层拓展 A：分配策略不止一种——first-fit / best-fit
...
## 11. 深层拓展 B：对齐（alignment）——分配里最容易忽略的约束
