...
>    方差恰好为 0（全等值输入）时除零。这是第 18 课数值精度的先声。
> 2. 为什么编译器**必须**在高层做这个下降，而不等到底层？
>    因为"reduce_mean + 逐元素"这个组合在底层的循环形态完全取决于
>    形状——**高层降一次，底层优化（第 11 课调度）才能针对正确形态**。
>    这就是 3.3 说"下降时机讲究"的机制层面的原因。
...
> **更深一层：看一段"编译后的图"长什么样**
>
> 第 0 课的 toycc 模型（融合后 4 个核）被 Relax 编译成 VM 后，
> 指令流大致是：
>
> ```
...
>
> 1. **内存规划在这里落地**（第 6 课）：`alloc/kill` 成对出现——
>    缓冲的复用策略被编进了指令流，而不是运行时临时想。
> 2. **融合的核是黑盒**（第 3 课）：`call_tir(fn_conv1, ...)` 一个函数
>    一次调用——中间 tensors 根本没出现（它们没逃出核）。
> 3. **指令是"解释执行"的**：所以 VM 天然支持动态形状/控制流——
>    遇到 `If`/`While` 变成跳转指令，形状可以在运行时才知道。
>
> 对比 toycc：`ReferenceEvaluator`（第 2 课）就是"解释执行"的最小版——
