...
> 形状、内存、布局全规划好。这就是"编译期 vs 运行时"二分法
...
> 而且是后面三件事的共同基础：
> - **代码生成**：生成的 C 代码就是按拓扑序一行行写的。
>   顺序错了，`bias_add1` 还没算出来，`relu1` 就去用它——直接编译报错或算错。
> - **内存规划**：要算"每个张量活到第几步"，前提是先有"第几步"
...
输入是 x 和 conv1_w"。`<- fused: add+relu` 是融合来源标记。
...
- 三个分析函数是 pass 的公共工具：`consumers`（谁吃我）、`topo_order`（谁先跑）、`dump`（画图）；新算子注册后无需为它们改代码
- 新增 pass 不动 IR；改动 IR 数据结构（如 `Node` 字段）才需要连带检查所有 pass
...
> 2) 它提前发现非法模型：没有这道检查，有环的图会让执行器在环上无限循环，
>    你只能对着卡死的运行去猜哪里出了环，远不如一条报错直接指出出错位置。
