...
> 计算本身一点没少，只是数据白搬了一趟。融合就是让这 20ns 归零。
>
> **为什么带宽比计算贵？** 因为硬件的算力增长远快于带宽增长。
...
> 真实 TVM 用 `op_pattern` 给每个算子标注这个"依赖形状"，
...
逼你去检查规则。融合 pass 的开发循环是：**写 pass → 数值验证 → 发现
语义被破坏 → 修规则**。
...
第二段就是刚才"先 relu 再加 bias"那个顺序错图真实跑出来的。
