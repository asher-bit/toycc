...
**下一步**：回到第 1 节的检查清单和第 3 节的六个方向——先对照清单
把没把握的项补上，再从方向 A~F 里选一个开始。遇到不懂的概念回
`glossary.md` 查；遇到读不懂的源码，用三遍读法 + 对照表法。
...
自研芯片岗位的深度要求来自任务本身：流片前四柱（模拟器/内存模型/
二进制/驱动，第 27~30 课）是同一套技术栈的四层，任缺一层就
"跑不起来、查不到底"。所以与其"什么都懂一点"，不如把
"硬件 + GPU 编译 + 工具链"这条线做深。这张地图的价值，
是让你**看清楚该在哪条线上加深**。
