...
> **原理深挖：为什么"会改代码"不是核心能力，"会验证"才是？**
>
> 你可能会觉得：任务 A 就是把 maxpool 登进去、写个实现——这有啥难的？
> 真正的难点在你**注意不到**的地方：
>
> - 你的 `RefImpl.maxpool` 写对了，但**建图时形状推断对了吗**？（`_maxpool_shape`）
> - maxpool 能不能被融合？**它和 conv 的依赖形状一样吗**？赶紧想……
> - 输出结果对了，但**如果写法不同（比如 stride 处理），隐藏的形状/边界 case
>   会在第 1000 次推理时才炸**。
>
> **验证（`max|Δ|` / 单测 / 随机测试）就是替你覆盖这些"没想到"的地方。**
> 一个编译器工程师估值，不看"能写多少行"，看"写完敢不敢拍胸脯说它对"——
> 而这个底气全来自验证。这就是为什么每个任务都强制带验证：
