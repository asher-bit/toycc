...
对照第 1 课 `conv` 的登记：`maxpool` 输入数 = 1（一个张量）；
...
**这一步错了，图跑起来就是错的**——形状是第 4 课布局、第 6 课内存规划的地基。
所以**先写形状推导，再写计算**（回看第 1 课为什么 `infer_shape` 和 `ref` 必须分开）。
...
**为什么必须用"直接按定义"的写法？** 第 2 课的原理：参考实现是要当裁判的，
...
**验证什么？三层：**
1. `max|Δ| ≈ 0` —— 数值对（第 2 课判据）
2. `dump` 输出里 maxpool 的形状符合预期 —— 形状对（第 1 课）
3. **融合规则有没有被误触发** —— 见 2.5
...
**数学等价性的证明**（第 3 课翻车现场就是反例）。做一遍这个分析，
...
第 6 课实现的是 first-fit（第一个死的就复用）。真实分配器还有 best-fit /
区间着色。任务：把 `memory.py` 的分配器改成 **best-fit**（选"死亡时间最接近"
的那个缓冲区），对比两种策略的峰值内存。

**验证**：两种策略的峰值应该一样（都是 256）——因为我们的模型缓是
"峰值为 256 的核"，但改完后你能看到算法差异在真实代码里的模样。
做完说一下：**为什么现实里选哪种策略要看"碎片化"**——这正是第 24 课
自研芯片工具链里要自己拍板的问题。
...
"变换函数"包装成 pass 管线的标准件（对照第 0 课"pass 是无副作用的工序"）：
输入一个 `func`，输出一个新 `func`，期间你可以自由打印、改写、替换。

**目标不是写出多牛的优化**，而是打通三个动作：
1. 注册一个 pass（`function_pass` 装饰器）
2. 遍历 IR 并打印（`PreOrderVisitor` —— 对照第 1 课 `consumers`/`topo_order`）
...
pass 有关、可能的修复方向"。用前 9 课的表去定位，你就已经有比很多人
清晰的视角。
...
做完这四周，再回课程主线（第 11 课调度）。关键不是快，是**每个环节都
亲手做一遍**。
