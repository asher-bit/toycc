...
> - 第二遍读入口：只跟 pass 工厂和顶层函数走一遍，弄清入口函数的每个参数负责什么（注册名、opt_level、输入输出）
...
1. 默写"三遍读法"：第一遍、第二遍、第三遍各读什么、各产出什么；
2. 把 `FuseOps()` 的三步（建图 → 划分 → 改写）与 toycc 的 `FusionPass` 逐项对照，各用一行写出差异；
3. 打开 `fuse_ops.cc`，用 grep 定位"后支配树"相关的函数名，说出它防的是哪类错误融合；
4. 把 toycc 的 `fold_constant` 与真实 `FoldConstant` 的"可折叠判定"各写三条，找出共同的一条与不同的一条。
