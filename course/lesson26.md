...
LLVM IR 太底层，表达不了"卷积/融合/布局"这些高层结构。
...
你手写融合时：遍历图、找模式、改写。MLIR 把这抽象成 **pattern rewrite**：
...
这就是 VM 里 `alloc_tensor` + `kill_tensor` 的 MLIR 版——
...
## 10. 扩展阅读 B：Interface——MLIR 的能力接口
...
**导航**：⬅ [上一节](lesson25.md)（第 25 课 · LLVM 深入）　｜　[下一节](lesson27.md)（第 27 课 · 模拟器与差分测试）➡
