...
一个 `(N=1, C=2, H=2, W=2)` 的 4 维张量，8 个元素。两种常见排法：

**NCHW**（把通道放前面，通道内连续）：

```
下标顺序:   n c h w
n=0, c=0:  h0w0 h0w1 h1w0 h1w1   ← c=0 的 4 个像素挨着
n=0, c=1:  h0w0 h0w1 h1w0 h1w1   ← c=1 的 4 个像素挨着
```

**NHWC**（把通道放最后，同一像素的通道连续）：

```
下标顺序:   n h w c
h0w0:  c0 c1              ← 同一像素的所有通道挨着
h0w1:  c0 c1
h1w0:  c0 c1
h1w1:  c0 c1
```

**两种排法存的"是同一个张量"（逻辑上），但内存里数值顺序完全不同。**
...
## 12. 深层拓展 A：strides——布局的数学表达（看懂一切排布的钥匙）
...
## 13. 深层拓展 B：打包布局（NCHW4c）——比轴交换更狠的排法
