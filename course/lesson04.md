...
CPU 的 SIMD 指令一次能对 8 个 float 做运算（AVX-256）。NHWC 里同一像素的
通道正好排在一起——只要通道数不超过 8（本节例子是 C=2），一条 SIMD 指令
就能把一个像素的全部通道一起算了。NCHW 则散着，不好向量化。

> **一句话直觉**：卷积内层循环是对"通道"求和。数据排法决定了这个内层循环
> 能不能"顺路"读、能不能一条指令算多个。这就是布局影响性能的本质。

**为什么 GPU 上反而常用 NCHW？** 因为 cuDNN 等库对 NCHW 和 NHWC 两种布局
都有对应 kernel，但 NCHW 版本历史更久、调优更充分，所以库的优化常盖过
排法本身的差异。所以：
...
> 搬移则是**真的重新排数据**：把 192 个元素按新顺序写一遍，耗时耗内存。
>
...
完成以下四项才算通过本课（每题都能用 `python -m course.runner 4` 验证）：

1. 手算：形状 `(1, 3, 8, 8)` 的张量在 NCHW 与 NHWC 两种布局下，元素 `(n=0, c=2, h=3, w=5)` 的偏移各是多少（各维 stride 自己定，写出公式再代入）；
2. 说出布局 pass 在图上做了什么（插了几个 layout_transform、为什么权重和输入都插），并解释"真正运行时只有 2 次边界搬移"这句话的依据；
3. 用一句话各写：改布局对"缓存友好"和"SIMD 向量化"各有什么好处；
4. 解释为什么"给权重转布局"的操作可以留给第 5 课折叠掉，而"给输入转布局"不行。
...
还记得本课前面 TVM `RewriteExpr` 里那个 `else` 分支吗？——"维数不同
