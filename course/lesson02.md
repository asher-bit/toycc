...
完成以下四项才算通过本课（每题都能用 `python -m course.runner 2` 验证）：

1. 用一句话说出参考执行器为什么能当"裁判"（提示：独立实现原则，和差分测试同源）；
2. 写出 `max|Δ|` 检查的完整步骤（喂输入 → 两个执行器各算一遍 → 逐元素差 → 取最大绝对值），并说明为什么不能只看"输出形状相同"；
3. 手算：两个数组 `[1.0, 2.0]` 与 `[1.0, 2.0001]` 的 max|Δ| 是多少；
4. 解释"固定随机种子"在两次执行对比中的作用，并举一个"不固定种子会怎样"的反例。
