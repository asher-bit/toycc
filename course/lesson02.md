...
融合进来的激活（`conv(x, w, bias)` + `activation="relu"`），
执行时在算子输出上**再套一层**：
...
你会在后面发现：常量折叠就是把这个执行器**搬到编译期**跑了一遍。
...
>   `1e-4 × 1000 = 0.1`——万分之一（0.01%）的相对差。对数值大的
>   激活值，绝对容差太苛刻，相对容差才反映"精度比例"。
>
> 手算两个极端：
> ```
...
- **Python 前端**能直接解释执行高层算子——本质上就是我们的 `ReferenceEvaluator`
- 编译完，官方教程的标准动作：
  ```python
...
## 9. 扩展阅读 A：浮点比较的科学（rtol / atol / NaN / 逐 bit）
...
- 输出值是 `1e+12` 时，误差 `1e-3` 都算很小——相对误差只有
  `1e-3 / 1e+12 = 1e-15`，比阈值 `1e-9` 还小 6 个数量级
...
这就是参考执行器的价值——它是**独立**的裁判：它不参与优化、不被任何
pass 改动，两边结果对不上时，差异一定来自优化（或参考实现本身）；
它只负责把"哪里不一样"摆出来，"为什么不一样"还得靠人查。
