...
显存利用率从 ~40% 提到 ~90%+：同批显存能装的请求数 ×2.25，带宽未到顶时吞吐才能跟涨。
...
FlashAttention-2/3 的两个新特性分别对应前面的机制：**FA-2 的
warp 级分工对应 warp 原语，FA-3 的 TMA 对应异步拷贝**。
...
**Q：这些账需要背吗？**
A：公式结构要背（三行公式），数字不用——会查表（HBM 带宽、算力）
