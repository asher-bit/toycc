...
- `k,i,j` 顺序：内层 j 扫 B 的一行，且每行只被完整读一遍 → 缓存友好
...
>  小例子里 ① 和 ② 的流量一样, 区别只是"每行拉进来当场用几格"：① 每条 B 行只用 1/16 格, ② 用满 16/16 格(折成拉进-已用总流量: 128/1088 ≈ 11.8% vs 100%)
>
...
现代 CPU 的 SSE 指令一次能对 4 个 float 做乘法（AVX-256 则是一次 8 个）。
把内层 4 个元素的循环**换成一条 SIMD 指令**，就省掉了 4 次"取数→算→存"中的
3 次指令开销——这是 4 倍速度提升的来源之一。
...
现在对于**每一个 k 值**，块内 16 个输出都只复用 A 中固定的 4 个元素
（A[i_o*4+i_i][k]，i_i 取 0..3）——A 的访问变成"4 个固定元素反复用"，
完美复用缓存。
...
只是循环怎么组织变了。这就是调度的铁律——和"融合必须语义等价"
是同一个原则在循环层的体现。
...
2. **自动调度（meta_schedule / autotuning）**：机器搜索，后面会专讲
...
对照"调度前/调度后"两段代码，数一数：循环从 3 层变成 5 层（i、j 各拆成
内外两层，+2），内层加了 SIMD 注释，外层加了并行指令。**语义没变，写法大变。**
...
这些 pass 的输入输出都是 TIR 程序。你读它们的方式，还是老一套：
...
然后在 TIR 层做调度，最后 codegen。`FuseTIR` 就是
