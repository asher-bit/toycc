...
> **手算：同样的计算，为什么顺序不同，缓存差距这么大？**
>
> 取 M=N=K=8 的 matmul，都是 float32（4 字节）。假设缓存行 64 字节（16 个 float），
> 每行缓存能装下矩阵的一行。访问 `B[k][j]` 时：
>
> **顺序 ① `i,j,k`**：内层循环按 `j=0,1,2,...` 扫 B 的第 k 行——
> 这是 B 的一行，本来连续，**没问题**。但再看 A：`A[i][k]` 按 `k` 扫 A 的一行，也连续。
> 等等，那问题在哪？——问题在下一次外层迭代进 `i` 时，**B 整行之前被 k 扫完，
> 再次进入时 B 已经不在缓存**（B 有 K×N=64 个元素 = 4 条缓存行，放得下啦？）。
> 找个更大的例子：M=N=K=64。B 是 64×64×4B = 16KB。L1 缓存假设 32KB——
> 其实也放得下。真正崩溃的是 **N=K=256**：B 是 256KB，远超 L1。
>
> 这时看 `i,j,k`：内层对 B 的每一列 `j` 访问 `B[k][j]`，按 k 扫是**按列跳**，
> 每次跳一行（64×4=256 字节），一个缓存行只取 4 字节就扔了——**缓存行 96% 浪费**。
> 而 `k,i,j` 顺序：内层 `j` 连续扫 B 的一行，一个缓存行的 16 个 float 全被用上。
>
