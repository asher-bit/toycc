...
> **手算：把循环"翻页"看——顺序为什么决定缓存**（像 LeetCode 一样逐页走，纸笔可复现）
>
> 舞台：M=N=K=16 的 matmul（float32，4 字节）。缓存行 = 64 字节 = 16 个 float，
> **所以 B 的一行正好 = 一条缓存行**。图例：`▣` 刚被搬进缓存　`◍` 已被用过　`·` 还在内存。
>
> ——— 顺序 ① `i,j,k`：内层是 k ———
>
> 第 1 页（i=0, j=0, k=0）：
> ```
>          B 矩阵（16×16）                内存视图：一行 = 一条缓存行
>        j=0 j=1  ...  j=15
> k=0   [▣   ·    ·    ·  ]        ┌┈┈┈┈┈┈┈┈┈┈┈┈┈┈┐
> k=1   [·    ·    ·    ·  ]        ┊▣ 只用了 1 格   ┊   ← 整条 64B 被拖进 L1
> k=2   [·    ·    ·    ·  ]        ┊· 0 格         ┊
>  ...                             ┊·              ┊
> k=15  [·    ·    ·    ·  ]        └┈┈┈┈┈┈┈┈┈┈┈┈┈┈┘
>          ↑
>   代码只想要 B[0][0] 这一个 float（4B）
>   但缓存按"行"给 → 拉到 64B, 用完 4B
> ```
>
> 第 2 页（k=1）：`B[1][0]` 在下一行 → **又**拉一条缓存行，**又**只用第 1 格。
> 第 3 页 = 快进到 k=15：16 条缓存行全被拉进来，每行都只用了第 1 格。
>
> 页面小结（k 整轮走完）：
> ```
> 拉到 L1:  16 条缓存行 = 1KB        真正用: 16 格 = 64B
> 利用率:   64 / 1024 = 6.25%       ← 每页都在重复"拉一整行, 用一格"
> ```
>
> ——— 换一种翻法：顺序 ② `k,i,j`：内层是 j ———
>
> 第 4 页（i=0, k=0，内层 j 从 0 扫到 15）：
> ```
>        j=0 j=1 j=2 ... j=15
> k=0   [◍◍◍◍◍◍◍◍◍◍◍◍◍◍◍◍]      ┌┈┈┈┈┈┈┈┈┈┈┈┈┈┈┐
> k=1   [·              ]        ┊◍ 16/16 全用完  ┊  ← 同一条缓存行
>  ...                          ┊·               ┊      喂饱 16 次乘加
>                                └┈┈┈┈┈┈┈┈┈┈┈┈┈┈┘
> A 这边: 内层 j 不动 k → A[i][k] 一直是 A[0][0], 一个值反复用, 零压力
> ```
>
> 第 5 页（k=1）：第二条缓存行，又是 16/16。
>
> 对比账：
> ```
>            顺序 ① i,j,k       顺序 ② k,i,j
> 同一轮计算   256 次乘加         256 次乘加      (完全一样)
> 拉进 L1     16 条缓存行        16 条缓存行      (完全一样)
> 用完的格数   16 (6.25%)        256 (100%)       ← 唯一区别
> ```
>
> ——— 那为什么 16×16 不崩，N=256 才崩？——
>
> 16×16 时 B 总共 1KB，**放得下 L1**：只有第一轮 j 支付了"拉整行走 1 格"的
> 浪费，之后全缓存命中——所以小例子看不出灾难，**但机制已经发生**。
>
> 放大到 M=N=K=256（B=256KB > L1 的 32KB）：
> ```
> ① i,j,k: 每个 (i,j) 都从主存重拉一列 ≈ M×N×K×64B = 1 GB   ← 每 4B 用 64B 的流量
> ② k,i,j: 每行拉进就被喂饱     ≈ M×K×256×4B  = 64 MB
> 同样计算量, 内存流量差 16 倍 → L1 越小、矩阵越大, 差距越狠
> ```
>
> **一句话**：顺序决定"一个缓存行里的数据，被用了几次"。
> 用满 16 次 = 传输 4 字节；只用 1 次 = 传输 64 字节——同样的计算量，
> 内存流量差 16 倍。这就是 reorder 是"性能差异的最大来源"的原因。
>
> 自己练：拿纸画一个 8×8 的格子（一条缓存行 = 两行），跟着第 1~5 页
> 手动翻一遍——"哪一行被拉进来、用了几格"这个直觉，翻两遍就有了。
