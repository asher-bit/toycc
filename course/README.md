...
> 31 课按 9 大类编排，另有一个“真实 TVM 源码精读”专题和 LLVM/MLIR 源码分章：**地基 → pass → 真实框架 → 上手进阶 → 硬件系统 → 知识地图 → GPU 专项 → LLVM/MLIR → 工具链四柱**。
> 前 20 课通用，第 21~30 课是**自研 GPU 芯片岗位专属**；真实 Relax Pass 源码放在第 ③ 部分的专题目录中。从第 15 课起阅读顺序
...
11. ⑨ 工具链四柱 (27~30)          ← 流片前的地基: 模拟器→并发→二进制→驱动
```
