...
> 30 课按 9 大类编排：**地基 → pass → 真实框架 → 上手进阶 → 硬件系统 → 知识地图 → GPU 专项 → LLVM/MLIR → Relax 源码精读**。
> 前 20 课通用，第 21~26 课是**自研 GPU 芯片岗位专属**，第 27~29 课进入真实 Relax Pass 源码精读。从第 15 课起阅读顺序
...
| 课 | 主题 | 学到什么 | 动手 |
|---|---|---|---|
| 27 | **FuseOps 源码详解** | 数据流图、post-dominator、union-find、函数封装 | 画出融合前后 IR |
| 28 | **FoldConstant 源码详解** | 编译期执行 TIR、常量传播、shape 处理、构建缓存 | 追踪一个 call_tir 折叠 |
| 29 | **经典 Relax Pass 学习路线** | 从 Normalize 到 Codegen 的源码阅读顺序 | 完成 Pass 依赖地图 |
...
10. ⑨ Relax Pass 源码精读 (27~29) ← 从真实 TVM 源码回到 Pass 实现细节
```
